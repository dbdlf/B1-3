...
| Action 1-B | Slack – Send a Message | #pass 채널에 "[합격] {이름} — {점수}점" |
| Action 2-A | Google Sheets – Add a Row | `불합격자` 탭에 기록 |
| Action 2-B | Slack – Send a Message | #fail 채널에 "[불합격] {이름} — {점수}점" |
...
3. 각 경로에 Google Sheets(Add a Row), Slack(Send a Message) 모듈을 연결하고 웹훅 데이터를 필드에 매핑
...
Zapier에서 Make의 Router에 해당하는 기능은 Paths(if/then 분기)다. 무료 플랜은 2단계 Zap(트리거 1 + 액션 1)만 지원해 각 분기 경로에 액션 2개를 붙이는 구성이 불가능하고 Webhooks by Zapier도 프리미엄 앱이므로, Professional 14일 무료 체험으로 구현했다. (상세 사유는 6장 참고)

| 단계 | 모듈 | 설정 |
|---|---|---|
| Trigger | Webhooks by Zapier – Catch Hook | 웹훅 URL 발급, JSON 수신 |
| Paths | Path A: `score > 79` / Path B: `score < 80` | 각 Path에 숫자 비교 조건 설정 (Path A는 조건 목록에 "Greater than or equal to"가 없어 `(Number) Greater than 79`로 설정) |
...
2. Paths 스텝 추가 → 각 Path의 조건 규칙(Path A: Number Greater than 79 / Path B: Number Less than 80) 설정
...
| 3 | 조건 분기·웹훅 지원 | 무료 플랜에서도 Router/Filter, Webhook 모두 사용 가능 | 무료 플랜은 2단계 Zap(트리거 1 + 액션 1)만 지원해 분기 경로마다 액션을 붙이는 구성이 불가능하고, Webhooks by Zapier는 프리미엄 앱이라 유료 플랜 전용 |
| 4 | 무료 플랜 범위 | 월 1,000크레딧(일반 모듈 실행 1회 = 1크레딧), 활성 시나리오 2개, 폴링형 트리거 최소 간격 15분(웹훅은 즉시) | 월 100태스크(액션 실행 1회 = 1태스크, 트리거·필터는 미차감), 2단계 Zap 한정, 폴링 15분 |
| 5 | 과금 단위 | 크레딧 단위 — 트리거를 포함한 모든 모듈 실행이 1크레딧씩 차감되어 스텝이 많으면 소모가 빠름 | 태스크 단위 — 액션만 차감되어 단순 워크플로우는 유리하나, 태스크당 단가가 Make보다 높음 |
...
| 7 | 연동 서비스 범위 | 약 3,000개 이상 앱. HTTP/Webhook 모듈로 미지원 서비스도 직접 연결 가능 | 약 9,000개 이상 앱으로 커넥터 수는 최다. 단, 일부 앱은 "Premium"으로 유료 플랜 전용 |
...
단점
- 무료 플랜은 2단계 Zap만 가능해 멀티스텝·분기 구성이 불가능하고, 웹훅은 프리미엄 앱으로 유료 플랜 전용
...
이번 과제처럼 조건 분기가 필수인 워크플로우를 무료로 구현해야 하는 상황에서는 Make가 명확히 유리했다. Zapier는 온보딩 경험은 가장 뛰어났지만 무료 플랜의 2단계 제한과 웹훅의 프리미엄 앱 지정 때문에 동일한 구성을 무료로 만들 수 없었다. 실무에서는 "단순 연동 몇 개 = Zapier, 복잡한 로직/비용 민감 = Make(또는 자가호스팅 n8n)"으로 선택하는 것이 합리적이라고 판단한다.

---

## 6. 과금 관련 정리 (제약 사항 대응)

- 유료 기능 사용 여부: Zapier 무료 플랜은 2단계 Zap(트리거 1 + 액션 1)만 지원하고 Webhooks by Zapier는 프리미엄 앱으로 지정되어 있어, Professional 14일 무료 체험을 사용했다. 결제는 발생하지 않았으며 체험 종료 전 다운그레이드 예정.
